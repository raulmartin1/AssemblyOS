...
.Lend_shift:
	...
	.global _gp_waitS
	@; Rutina para bloquear un proceso.
	@;Parametros
	@; R0: semaforo en el que el proceso se bloqueara (0..7)
	@;Resultado
	@; R0: 0 si no se ha bloqueado, 1 si se ha bloqueado
_gp_waitS:
	push {r1-r2, lr}
	...
	@; Bloquear proceso en el semaforo
	bl _gp_inhibirIRQs
	mov r2, #0				@; en caso de estar a 1 (libre), mover un 0 en R2
	strb r2, [r1, r0]		@; y guardarlo en el semaforo indicado por parametro para cerrarlo

	ldr r1, =_gd_pidz		@; cargar direccion de _gd_pidz
	ldr r2, [r1]			@; obtener su valor
	orr r2, #0x80000000		@; poner el bit de mas peso a 1 para no ponerlo en la cola RDY
	str r2, [r1]			@; guardar el valor modificado

	ldr r1, =_gd_qBlock		@; cargar direccion de la lista BLK
	and r2, #0xF			@; obtener el zocalo actual
	strb r2, [r1, r0]		@; guardar zocalo en la lista BLK en la posicion indicada por el semaforo (parametro)

	ldr r1, =_gd_nBlock		@; cargar direccion del numero de procesos en la lista BLK
	ldr r2, [r1]			@; obtener su valor
	add r2, #1				@; sumarle 1
...
.LreturnWait:
	...
	.global _gp_signalS
	@; Rutina para desbloquear un proceso.
	@;Parametros
	@; R0: semaforo del proceso a desbloqueaar (0..7)
	@;Resultado
	@; R0: 0 si no se ha desbloqueado, 1 si se ha desbloqueado
_gp_signalS:
	push {r1-r4, lr}
	...
	ldr r1, =_gd_qBlock		@; cargar direccion de la lista BLK
	ldrb r2, [r1, r0]		@; extraer zocalo de la lista BLK en la posicion indicada por el semaforo (parametro)
	mov r3, #0				@; mover un 0 en R3
	strb r3, [r1, r0]		@; y guardarlo en la posicion de la lista BLK para restablecer su valor
	ldr r1, =_gd_qReady		@; cargar direccion de la cola RDY
	ldr r3, =_gd_nReady		@; cargar direccion del numero de procesos en RDY
	ldr r4, [r3]			@; obtener su valor
	strb r2, [r1, r4]		@; guardar zocalo en la cola RDY
	add r4, #1				@; sumar 1 al numero de procesos RDY
	str r4, [r3]			@; y guardarlo de nuevo

	ldr r3, =_gd_nBlock		@; cargar direccion del numero de procesos en la lista BLK
	ldr r4, [r3]			@; obtener su valor
	sub r4, #1				@; restarle 1
	str r4, [r3]			@; y guardarlo de nuevo
	bl _gp_desinhibirIRQs
