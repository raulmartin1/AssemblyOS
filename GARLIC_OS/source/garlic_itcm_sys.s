...
.section .dtcm,"wa",%progbits
	...
	.global _gs_colZoc		@; colores de las zonas de memoria de cada zocalo
_gs_colZoc: .byte 0, 224, 184, 128, 48, 208, 80, 112, 96, 160, 245, 144, 176, 64, 192, 32

	.global _gs_charTabla	@; codigos de los caracteres para dibujar la tabla de procesos
_gs_charTabla:
...
.section .itcm,"ax",%progbits
	...
	@; permite convertir un numero natural de 32 bits a su representacion
	...
	@; R2: unsigned int num
	@;Resultado
	@; R0: 0 si no hay problema, !=0 si el numero no cabe en el string
_gs_num2str_dec:
	push {r1-r8, lr}
	cmp r1, #1
	bhi .Ln1s_cont1			@; verificar si hay espacio para centinela + 1 digito
	mov r0, #-1
	b .Ln1s_fin				@; retornar con codigo de error en R0
.Ln1s_cont1:
	mov r6, r0				@; R6 = puntero a string de resultado
	mov r7, r1				@; R7 = indice caracter (inicialmente es la longitud del string)
	mov r8, r2				@; R8 = numero a transcribir
	mov r3, #0
	sub r7, #1
	strb r3, [r6, r7]		@; guardar final de string (0) en ultima posicion
.Ln1s_while:	
	...
	mov r2, sp				@; pasar direccion para albergar el cociente
	add r3, sp, #4			@; pasar direccion para albergar el resto
	bl _ga_divmod
	pop {r4-r5}				@; R4 = cociente, R5 = resto
	add r5, #48				@; anadir base de codigos ASCII para digitos numericos
	sub r7, #1
	strb r5, [r6, r7]		@; almacenar codigo ASCII en vector
	mov r8, r4				@; actualizar valor del numero a convertir
	cmp r8, #0				@; repetir mientras el numero sea diferente de 0
	bne .Ln1s_while
.Ln1s_pad:
	...
	mov r3, #' '
	sub r7, #1
	strb r3, [r6, r7]		@; almacenar codigo ASCII ' ' en vector
	b .Ln1s_pad
.Ln1s_cont2:
...
_gs_num2str_hex:
	push {r1-r4, lr}
	cmp r1, #1
	bhi .Ln2s_cont1			@; verificar si hay espacio para centinela + 1 digito
	mov r0, #-1
	b .Ln2s_fin				@; retornar con codigo de error en R0
.Ln2s_cont1:
	mov r3, #0
	sub r1, #1
	strb r3, [r0, r1]		@; guardar final de string (0) en ultima posicion
.Ln2s_while:	
	cmp r1, #0				@; repetir mientras quede espacio en el string
	beq .Ln2s_cont2
	and r4, r2, #0x0F		@; obtener el digito hexa de menos peso
	cmp r4, #10				@; si digito hexa menor que 10, saltar a tratamiento
	blo .Ln2s_dec			@; de digitos decimales
	add r4, #7				@; ajuste para letras 'A'..'F' (65 - 10 - 48)
.Ln2s_dec:
	add r4, #48				@; anadir base de codigos ASCII para numeros
	sub r1, #1
	strb r4, [r0, r1]		@; almacenar codigo ASCII en vector
	mov r2, r2, lsr #4		@; actualizar valor del numero a convertir
	cmp r2, #0				@; repetir mientras el numero sea diferente de 0
	bne .Ln2s_while
.Ln2s_pad:
	...
	mov r3, #'0'
	sub r1, #1
	strb r3, [r0, r1]		@; almacenar codigo ASCII '0' en vector
	b .Ln2s_pad
.Ln2s_cont2:
	mov r0, r2				@; esto indicara si el numero se ha podido codificar
.Ln2s_fin:					@; completamente en el string (si R0 = 0)
	...
	.global _gs_copiaMem
	@; copia un bloque de memoria desde una direccion fuente a otra direccion
	@; destino, el numero de bytes indicado;
	@;Parametros:
	@; R0: direccion fuente (debe ser multiplo de 4)
	@; R1: direccion destino (debe ser multiplo de 4)
	@; R2: numero de bytes a copiar
_gs_copiaMem:
	push {r0-r12, lr}
	and r11, r2, #3				@; R11 = contador de bytes residuales
	mov r2, r2, lsr #2			@; convierte numero bytes en numero words
