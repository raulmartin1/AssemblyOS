...
intFunc _gm_cargarPrograma(char *keyName)
{
	...
	if (n_entradas != 0)
	{
		fseek(ficher, offset, SEEK_SET);						/*Apuntem al program header del ficher (despla�ament de la taula de segments)*/
	...
	for(i=0;n_entradas>i;i++)
	{
		...
		if(t_segment == 1)						/*Si es 1 es del tipus PT_LOAD*/
		{
			...
			/*Mirem que pel seg�ent programa el _gm_prim_pmem_free sigui multiple de 4*/
			int s = mida_seg%4;
			if(s!=0){
			...
			/*Actualitzem _gm_prim_pmem_free per al seg�ent programa*/
